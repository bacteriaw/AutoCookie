...
async function getCookie() {
    try {
        ...
        if (!(headers['cookie'] && Body?.data?.mobile)) throw new Error("获取token失败！参数缺失");
        //上传ck
        await refreshQingLong("SMZDM_COOKIE", headers['cookie'], Body?.data?.mobile);
    } catch (e) {
        await refreshQingLong("SMZDM_COOKIE");
    }
}
