...
async function userinfo() {
    try {
        let url = {
            url: `${hostname}/apios/appuserinfo.ashx`,
            headers: {
                "Host": host,
        ...
        let result = await httpPost(url, `查询信息`);
...
async function signin() {
    try {
        let url = {
            url: `${hostname}/ajax/qiandao.ashx`,
            headers: {
                "Host": host,
                "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
            },
            body: "action=checkinTaskCenter&usertoken=" + ck
...
async function timing() {
    try {
        let url = {
            url: `${hostname}/app/UserActionMonitorAjax.ashx`,
            headers: {
                "Host": host,
                "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
            },
            body: "actiontype=302&sign=a8dc24fd&taskSysNo=0&coins=0&signdate=20220914182616&f=android&code=581&uid=01e0add6-3d53-4182-9303-ff08643147ce&usertoken=" + ck + "&v=5.8.1&platform=XiaoMi&type=4&isFromWeb=13"
        };
        let result = await httpPost(url, `领取定时奖励`);

        console.log(result);
        if (result?.error_code == 0) {
            if (result.data.time == 0) {
                console.log("当前时间段领取成功了呢");
            } else {
                console.log("当前时间段奖励已经领取了呢,请修改定时时间哦~");
            }
            await wait(3);
        } else {
